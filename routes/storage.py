...
MAX_FILE_SIZE = 50 * 1024 * 1024  # 50MB quota
...
@login_required
def upload_file():
    """Handle upload file dengan validasi keamanan"""
    ...
    # Hindari overwrite file yang sama (tambahkan timestamp jika perlu)
    counter = 1
...
def download_file(filename):
    """Download file milik user sendiri saja"""
    user_folder = get_user_folder()
    safe_path = os.path.join(user_folder, filename)
...
def delete_file(filename):
    """Hapus file milik user sendiri saja"""
    user_folder = get_user_folder()
    safe_path = os.path.join(user_folder, filename)
